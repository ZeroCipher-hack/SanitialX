...
class PostgresSoarRepository:
    ...
    async def queue_agent_command(self, action: SoarActionModel, *, agent_id: str, command_type: str, payload: dict, actor: str) -> AgentCommandModel:
        ...
        command = AgentCommandModel(command_id=str(uuid.uuid4()), agent_id=agent_id, action_id=action.action_id, command_type=command_type, payload=payload, status="QUEUED", result={})
    ...
    async def pending_agent_commands(self, agent_id: str, *, limit: int = 20) -> list[AgentCommandModel]:
        result = await self._session.execute(select(AgentCommandModel).where(AgentCommandModel.agent_id == agent_id, AgentCommandModel.status == "QUEUED").order_by(AgentCommandModel.created_at).limit(limit))
        return list(result.scalars().all())

    async def claim_agent_command(self, command: AgentCommandModel) -> AgentCommandModel:
        if command.status == "QUEUED":
            command.status = "CLAIMED"; command.claimed_at = datetime.now(timezone.utc)
    ...
    async def complete_agent_command(self, command: AgentCommandModel, *, result: dict) -> AgentCommandModel:
        ...
        command.result = result; command.completed_at = datetime.now(timezone.utc)
        ...
        self._session.add(SoarAuditModel(audit_id=str(uuid.uuid4()), action_id=command.action_id, actor=f"agent:{command.agent_id}", event=command.status, details={"command_id":command.command_id, **result}))
        await self._session.commit(); await self._session.refresh(command); return command
