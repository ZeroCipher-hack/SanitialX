...
SAFE_AGENT_COMMANDS = {"COLLECT_SYSTEM_INFO", "REFRESH_INVENTORY"}
...
async def _require_agent(repo:PostgresAgentRepository,agent_id:str,token:str|None):
    ...
    if model is None:raise HTTPException(status_code=404,detail="Agent not found.")
    if not token or not verify_agent_token(token,model.agent_token_hash):raise HTTPException(status_code=401,detail="Invalid agent token.")
    return model

def _version_policy(agent:Any)->dict[str,object]:
    return evaluate_agent_version(agent_id=agent.agent_id,os_name=agent.os,current_version=agent.agent_version,settings=get_settings()).to_dict()
...
async def enroll_agent(payload:AgentEnrollmentPayload,session:Annotated[AsyncSession,Depends(get_db_session)],enrollment_key:Annotated[str|None,Header(alias="X-Enrollment-Key")]=None)->dict[str,Any]:
    ...
    if not enrollment_key or not hmac.compare_digest(enrollment_key,settings.api_key):raise HTTPException(status_code=401,detail="Invalid enrollment key.")
...
async def agent_heartbeat(agent_id:str,payload:AgentHeartbeatPayload,session:Annotated[AsyncSession,Depends(get_db_session)],agent_token:Annotated[str|None,Header(alias="X-Agent-Token")]=None)->dict[str,Any]:
    repo=PostgresAgentRepository(session);await _require_agent(repo,agent_id,agent_token);updated=await repo.heartbeat_agent(agent_id,cpu_usage=payload.cpu_usage,memory_usage=payload.memory_usage,agent_version=payload.agent_version,ip_address=payload.ip_address);assert updated is not None
    settings=get_settings();return {"agent_id":updated.agent_id,"status":updated.status,"last_seen":updated.last_seen.isoformat(),"next_heartbeat_seconds":settings.agent_heartbeat_interval_seconds,"version_policy":_version_policy(updated)}

@router.get("/{agent_id}/commands")
async def agent_commands(agent_id:str,session:Annotated[AsyncSession,Depends(get_db_session)],agent_token:Annotated[str|None,Header(alias="X-Agent-Token")]=None,limit:int=Query(default=10,ge=1,le=20))->dict[str,Any]:
    agent_repo=PostgresAgentRepository(session);await _require_agent(agent_repo,agent_id,agent_token);repo=PostgresSoarRepository(session);commands=await repo.pending_agent_commands(agent_id,limit=limit);out=[]
    for command in commands:
        if command.command_type not in SAFE_AGENT_COMMANDS:continue
        command=await repo.claim_agent_command(command);out.append({"command_id":command.command_id,"command_type":command.command_type,"payload":command.payload,"created_at":command.created_at})
    return {"agent_id":agent_id,"commands":out}

@router.post("/{agent_id}/commands/{command_id}/result")
async def agent_command_result(agent_id:str,command_id:str,payload:AgentCommandResult,session:Annotated[AsyncSession,Depends(get_db_session)],agent_token:Annotated[str|None,Header(alias="X-Agent-Token")]=None)->dict[str,Any]:
    agent_repo=PostgresAgentRepository(session);await _require_agent(agent_repo,agent_id,agent_token);repo=PostgresSoarRepository(session);command=await session.get(__import__('db.models.soar',fromlist=['AgentCommandModel']).AgentCommandModel,command_id)
    if command is None or command.agent_id!=agent_id:raise HTTPException(status_code=404,detail="Agent command not found.")
    if command.command_type not in SAFE_AGENT_COMMANDS:raise HTTPException(status_code=409,detail="Command type is not enabled for endpoint execution.")
    try:command=await repo.complete_agent_command(command,result=payload.model_dump())
    except ValueError as exc:raise HTTPException(status_code=409,detail=str(exc)) from exc
    return {"command_id":command.command_id,"status":command.status,"completed_at":command.completed_at,"result":command.result}
...
async def agent_replace_software_inventory(agent_id:str,payload:SoftwareInventoryPayload,session:Annotated[AsyncSession,Depends(get_db_session)],agent_token:Annotated[str|None,Header(alias="X-Agent-Token")]=None)->dict[str,Any]:
    agent_repo=PostgresAgentRepository(session);await _require_agent(agent_repo,agent_id,agent_token);items=await PostgresVulnerabilityRepository(session).replace_software_inventory(agent_id,[item.model_dump() for item in payload.software]);asset=await agent_repo.touch_inventory(agent_id);return {"agent_id":agent_id,"software_count":len(items),"inventory_updated_at":asset.get("inventory_updated_at") if asset else None,"lifecycle_status":asset.get("lifecycle_status") if asset else None}

@router.post("/{agent_id}/events",status_code=202)
async def agent_forward_events(agent_id:str,payload:AgentEventBatch,session:Annotated[AsyncSession,Depends(get_db_session)],agent_token:Annotated[str|None,Header(alias="X-Agent-Token")]=None)->dict[str,Any]:
    agent_repo=PostgresAgentRepository(session);agent=await _require_agent(agent_repo,agent_id,agent_token);bus=RedisEventBus.from_url(get_settings().redis_url)
    try:
    ...
    await agent_repo.record_forwarded_events(agent_id,len(payload.events));return {"agent_id":agent_id,"accepted":len(payload.events),"pipeline":"redis-stream -> detection -> correlation -> incident"}
...
@router.get("/summary")
async def get_asset_summary(session:Annotated[AsyncSession,Depends(get_db_session)],_user:Annotated[TokenPayload,Depends(get_current_user)],stale_after_hours:int=Query(default=24,ge=1,le=720))->dict[str,int]:return await PostgresAgentRepository(session).get_inventory_summary(stale_after_hours=stale_after_hours)
...
async def get_asset_detail(agent_id:str,session:Annotated[AsyncSession,Depends(get_db_session)],_user:Annotated[TokenPayload,Depends(get_current_user)])->dict[str,Any]:
    ...
    if asset is None:raise HTTPException(status_code=404,detail=f"Asset '{agent_id}' not found.")
...
async def update_asset_metadata(agent_id:str,payload:AssetMetadataUpdate,session:Annotated[AsyncSession,Depends(get_db_session)],_user:Annotated[TokenPayload,Depends(require_role(["admin","analyst"]))])->dict[str,Any]:
    ...
    if updated is None:raise HTTPException(status_code=404,detail=f"Asset '{agent_id}' not found.")
    return updated
...
async def replace_software_inventory(agent_id:str,payload:SoftwareInventoryPayload,session:Annotated[AsyncSession,Depends(get_db_session)],_user:Annotated[TokenPayload,Depends(get_current_user)])->dict[str,Any]:
    agent_repo=PostgresAgentRepository(session)
    if await agent_repo.get_agent(agent_id) is None:raise HTTPException(status_code=404,detail=f"Agent '{agent_id}' not found.")
    items=await PostgresVulnerabilityRepository(session).replace_software_inventory(agent_id,[item.model_dump() for item in payload.software]);asset=await agent_repo.touch_inventory(agent_id);return {"agent_id":agent_id,"software_count":len(items),"inventory_updated_at":asset.get("inventory_updated_at") if asset else None,"lifecycle_status":asset.get("lifecycle_status") if asset else None,"software":[_software_to_dict(item) for item in items]}
