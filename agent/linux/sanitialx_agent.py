...
SSH_DISCONNECT_RE = re.compile(r"(?:Disconnected from|Connection closed by) (?:invalid user )?(?P<user>\S+ )?(?P<ip>[0-9a-fA-F:.]+)")
JOURNAL_CURSOR_RE = re.compile(r"^-- cursor: (?P<cursor>.+)$")
...
def memory_percent() -> float:
    try:
        ...
        return round(100 * (total - available) / total, 2) if total else 0.0
    except Exception:
        return 0.0
...
class Agent:
    def __init__(self, server: str, state_dir: Path) -> None:
        ...
        self.ssh_journal_cursor_path = state_dir / "ssh-journal-cursor"
    ...
    def poll_commands(self) -> list[dict[str, Any]]:
        return list(
            request_json(
                "GET",
                f"{self.server}/agents/{self.agent_id}/commands",
                headers=self.agent_headers(),
                timeout=20,
            ).get("commands", [])
        )

    def report_command(self, command_id: str, result: dict[str, Any]) -> dict[str, Any]:
        return request_json(
            "POST",
            f"{self.server}/agents/{self.agent_id}/commands/{command_id}/result",
            result,
            self.agent_headers(),
            timeout=30,
        )
    ...
    def execute_command(self, command: dict[str, Any]) -> dict[str, Any]:
        ...
        if command_type not in SAFE_COMMANDS:
            return {"ok": False, "message": "Command type is not allowed by Linux agent whitelist.", "data": {}}
        if command_type == "COLLECT_SYSTEM_INFO":
            return {"ok": True, "message": "Read-only system information collected.", "data": self.system_info()}
        inventory_result = self.inventory()
        return {
            "ok": True,
            "message": "Software inventory refreshed.",
            "data": {
                "software_count": inventory_result.get("software_count", 0),
                "inventory_updated_at": inventory_result.get("inventory_updated_at"),
            },
        }
    ...
    def process_commands(self) -> int:
        ...
        for command in self.poll_commands():
            ...
            try:
                ...
                result = {"ok": False, "message": f"Safe command failed: {exc}", "data": {}}
    ...
    def _classify_auth_line(self, line: str):
        ...
        if match:
            return "SSH_AUTH_FAILURE", "MEDIUM", match.group("ip"), {
                "user": match.group("user"),
                "auth_result": "failed",
            }
        match = SSH_ACCEPTED_RE.search(line)
        if match:
            return "SSH_AUTH_SUCCESS", "INFO", match.group("ip"), {
                "user": match.group("user"),
        ...
        if match:
            return "SSH_INVALID_USER", "LOW", match.group("ip"), {
                "user": match.group("user"),
    ...
    def _queue_classified_auth(self, line: str, log_source: str) -> int:
        ...
        self.queue_event(
            ...
            line.strip(),
            source_ip=source_ip,
            metadata={"log_source": log_source, **metadata},
        )
        return 1

    def collect_auth_file_events(self, path: Path, max_lines: int = 2000) -> int:
        stat = path.stat()
        ...
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            ...
            for index, line in enumerate(handle):
                if index >= max_lines:
                    break
                queued += self._queue_classified_auth(line, str(path))
            new_offset = handle.tell()
        secure_write(self.auth_cursor_path, json.dumps({"inode": stat.st_ino, "offset": new_offset}) + "\n")
        return queued

    def _journal_cursor_from_output(self, output: str) -> str | None:
        for line in reversed(output.splitlines()):
            match = JOURNAL_CURSOR_RE.match(line.strip())
            if match:
                return match.group("cursor")
        return None

    def _initialize_ssh_journal_cursor(self) -> bool:
        try:
            proc = subprocess.run(
                ["journalctl", "-u", "ssh", "--no-pager", "--show-cursor", "-n", "0"],
                text=True,
                capture_output=True,
                check=True,
                timeout=15,
            ...
            return False
        cursor = self._journal_cursor_from_output(proc.stdout)
        if cursor:
            secure_write(self.ssh_journal_cursor_path, cursor + "\n")
            return True
        return False

    def collect_journal_auth_events(self, max_lines: int = 2000) -> int:
        if not Path("/run/systemd/system").exists():
            return 0
        if not self.ssh_journal_cursor_path.exists():
            self._initialize_ssh_journal_cursor()
            return 0
        cursor = self.ssh_journal_cursor_path.read_text().strip()
        if not cursor:
            return 0
        try:
            proc = subprocess.run(
        ...
        queued = 0
        journal_lines = proc.stdout.splitlines()
        records_seen = 0
        for line in journal_lines:
            if records_seen >= max_lines:
                break
            ...
            if not message:
                continue
            queued += self._queue_classified_auth(message, "journald:ssh.service")

        new_cursor = self._journal_cursor_from_output(proc.stdout)
        if new_cursor:
            secure_write(self.ssh_journal_cursor_path, new_cursor + "\n")
        return queued

    def collect_auth_events(self, max_lines: int = 2000) -> int:
        path = _find_auth_log()
        if path is not None:
            return self.collect_auth_file_events(path, max_lines=max_lines)
        return self.collect_journal_auth_events(max_lines=max_lines)
    ...
    def collect_system_events(self, max_lines: int = 200) -> int:
        ...
        if not self.journal_cursor_path.exists():
            ...
            return 0
        try:
            since_epoch = max(0, int(self.journal_cursor_path.read_text().strip()))
        except ValueError:
    ...
    def flush_events(self, batch_size: int = 200) -> int:
        ...
        request_json(
            "POST",
            f"{self.server}/agents/{self.agent_id}/events",
            {"events": events},
            self.agent_headers(),
            timeout=30,
        )
...
def main() -> int:
    ...
    for name in ("heartbeat", "inventory", "collect", "flush", "commands", "run"):
        sub.add_parser(name)
    ...
    try:
        if args.command == "enroll":
            ...
            print(json.dumps({"processed": agent.process_commands()}))
